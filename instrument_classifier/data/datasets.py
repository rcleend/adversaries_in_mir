...
from instrument_classifier.utils.paths import csv_path
from instrument_classifier.utils.utils import read_file
from instrument_classifier.data.feature_extraction import get_torch_spec, normalise
from instrument_classifier.data.data_utils import get_train_label_dict, get_test_label_dict, \
...
class AudioDataset(Dataset):
    ...
    def __init__(self, filename, data_path, feature_dict, norm_file_path=None, d_path_backup=None, valid=True):
        ...
        self.label_map = {l: i for i, l in enumerate(sorted(get_unique_label_set()))}
        self.filenames = sorted(list(list(self.filenames).intersection(set(self.labels.keys()))))
