...
config = AttrDict({
    'experiment_name': 'test_app',  # name to use for saving adversaries
...
def argument_parsing():
    ...
    parser = ArgumentParser(description='Program to compute hubness adversaries (approx)')
...
def check_for_hubs(new_gaussians, indices, gaussians, kl_divs, kl_thres, not_convs, old_k_occs):
    ...
    for cur in range(len(new_gaussians)):
        ...
        # count how often we are below threshold
        neigh_approx = torch.sum(torch.tensor([kl <= kl_thres[i] for i, kl in enumerate(new_kl)])) - 1
        if neigh_approx < config.hub_size * 5 - 1:
            # approximation too low yet
            new_update_mask.append(1.), k_occs.append(old_k_occs[cur])
            continue
        ...
        del old_kl
        if neigh_approx > k_occ:
            print('check: {} / {} / {}'.format(0. if idx in new_hubs else 1., neigh_approx, k_occ))
...
def compute_adversaries_batch(target_data, indices, middles, gaussians, kl_divs, kl_thres, init_hubs, pot_target_hubs, sp_model):
    ...
    for e in range(config.max_epochs):
        ...
        if e % 10 == 0:
            update_mask, k_occs = check_for_hubs(cpu_model((middles + delta).detach().cpu()), indices,
                                                 gaussians, kl_divs, kl_thres, update_mask, k_occs)
...
def compute_adversaries(files, adv_path):
    ...
    # prepare kl-div estimates for hubness filter-and-refine
    k_neigh_dists, _ = torch.topk(kl_divs, k=6, dim=-1, largest=False)
    kl_div_thres = {h: k_neigh_dists[h][-1].item() for h in range(len(kl_divs))}
    ...
    for b, (indices, middles) in enumerate(data):
        ...
        convs, perts, k_occs = compute_adversaries_batch(target_data, indices, middles, gaussians, kl_divs,
                                                        kl_div_thres, init_hubs, pot_target_hubs, sp_model)
